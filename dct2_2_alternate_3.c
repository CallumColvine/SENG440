...
#define ROOT2COS6 554 // root 2 * cos(6 pi / 16) * 1024 where 1024 == 2^10 i.e << 10
#define ROOT2SIN6 1338 
...
void dct(int X[ARRAY_SIZE][ARRAY_SIZE], int x[ARRAY_SIZE][ARRAY_SIZE])
{
	...
	for (i = 0; i < 8; i++)
	{
	 ...
	 // 	temp = x2;
        ...
        // His begin
        // REUSE x1-4 and temp to save variables
        // Note: can conver z's to x [0-3]

        // Stage 2 bottom rotators
        x0 = COS3 * (x7 + x4);
        temp = x4;
        x4 = (SIN3 - COS3) * x7 + x0;
        x7 = (-SIN3 - COS3) * temp + x0;

        x0 = COS1 * (x6 + x5);
        temp = x5;
        // printf("%i\n", x5);
        x5 = (SIN1 - COS1) * x6 + x0;
        x6 = (-SIN1 - COS1) * temp + x0;
        // printf("%i\n", x5);

        // Stage 3 top rotator
        x0 = ROOT2COS6 * (x3 + x2);
        temp = x2;
        x2 = (ROOT2SIN6 - ROOT2COS6) * x3 + x0;
        x3 = (-ROOT2SIN6 - ROOT2COS6) * temp + x0;

        // Stage 3 bottom butterfly
        temp = x4;
        x4 += x6;
        x6 = temp - x6;
        temp = x7;
        ...
        // Stage 4 bottom butterfly
        temp = x7;
        x7 += x4;
        x4 = temp - x4;
        x5 = x5 * ROOT2; // POSSIBLE ERROR. NEED TO SCALE MORE CAUSE
        x6 = x6 * ROOT2;


        // Assign values
		x[i][1] = (int)(x7 + 0.5f) >> 10;
		x[i][3] = (int)(x5 + 0.5f) >> 17; 	// Both mult by upscale root 2
		x[i][5] = (int)(x6 + 0.5f) >> 17; 	// Both mult by upscale root 2
		x[i][7] = (int)(x4 + 0.5f) >> 10;
		x[i][2] = (int)(x2 + 0.5f) >> 10;
		x[i][6] = (int)(x3 + 0.5f) >> 10;
	...
	for (i = 0; i < 8; i++)	
	{
        ...
        // x5 = x5 * ROOT2 >> 7; // POSSIBLE ERROR. NEED TO SCALE MORE CAUSE
        // x6 = x6 * ROOT2 >> 7;

        // His end

        // Assign values
		X[1][i] = (int)(x7 + 0.5f) >> 10 >> 3;
		X[3][i] = (int)(x5 + 0.5f) >> 17 >> 3; 	// Both mult by upscale root 2
		X[5][i] = (int)(x6 + 0.5f) >> 17 >> 3; 	// Both mult by upscale root 2
		X[7][i] = (int)(x4 + 0.5f) >> 10 >> 3;
		X[2][i] = (int)(x2 + 0.5f) >> 10 >> 3;
		X[6][i] = (int)(x3 + 0.5f) >> 10 >> 3;
	}
}
